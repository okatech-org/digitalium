...
import { CLASSEUR_TEMPLATES, CLASSEUR_COLORS, CLASSEUR_ICONS } from '../constants';

interface NewClasseurModalProps {
    isOpen: boolean;
    onClose: () => void;
    onCreate: (data: Partial<IClasseur>) => void;
}

export function NewClasseurModal({
    isOpen,
    onClose,
    onCreate
}: NewClasseurModalProps) {
    const [formData, setFormData] = useState<Partial<IClasseur>>({
        name: '',
        description: '',
        icon: '📚',
        color: 'bg-blue-500',
    ...
    const handleSubmit = () => {
        ...
        onClose();
        setFormData({ name: '', description: '', icon: '📚', color: 'bg-blue-500', is_system: false });
    };

    const applyTemplate = (template: typeof CLASSEUR_TEMPLATES[0]) => {
        setFormData({
            ...formData,
            name: template.name,
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-2xl border-border/40 shadow-2xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Créez un classeur pour organiser vos dossiers et documents.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {CLASSEUR_TEMPLATES.map((template, i) => (
                            <button
                                key={i}
                ...
                <div className="grid grid-cols-2 gap-6 py-4">
                    <div className="space-y-4">
                        {/* Name */}
                        <div className="space-y-2">
                            <Label>Nom du classeur *</Label>
                            <Input
                                placeholder="Ex: Gestion 2024, Projet Client..."
                                value={formData.name}
                        ...
                        <div className="space-y-2">
                            ...
                            <Textarea
                                placeholder="Décrivez le contenu de ce classeur..."
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="bg-muted/30 p-4 rounded-lg border border-border/40">
                            ...
                            <div className="flex items-center gap-3">
                                <div className={`w-14 h-16 rounded-lg ${formData.color} flex items-center justify-center shadow-lg relative overflow-hidden`}>
                                    {/* Spine effect */}
                                ...
                                <div>
                                    <p className="font-semibold text-foreground">
                                        {formData.name || 'Nouveau Classeur'}
                                    </p>
